...
@click.option("--provider", type=click.Choice(["openai", "anthropic"]), help="Set LLM provider")
...
@click.option("--output", type=click.Choice(["briefing", "flashcards", "audio-script", "all"]), 
              default="briefing", help="Output format")
@click.option("--preset", type=click.Choice(["general", "runbook", "changelog"]), 
              default="general", help="Content preset")
...
def ingest(source: str, output: str, preset: str, save: Optional[str]):
    ...
    try:
        ...
        if not config_obj.api_key:
            console.print("❌ No API key configured. Run: climber config set --api-key <key>", style="red")
            raise click.Abort()
        ...
        with Progress(
            ...
            for output_type, result in results.items():
                ...
                if save:
                    ...
                    output_file = save_path / f"{output_type}.{formatter.file_extension}"
                    output_file.write_text(formatted)
                    console.print(f"✓ Saved {output_type} to {output_file}", style="green")
                else:
